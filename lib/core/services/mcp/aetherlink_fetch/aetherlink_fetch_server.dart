...
/// @aetherlink/fetch — In-memory MCP server engine and transport (Flutter/Dart)
///
/// Provides four tools:
...
class AetherLinkFetchRequestPayload {
  final Uri url;
  final Map<String, String> headers;

  AetherLinkFetchRequestPayload({required this.url, Map<String, String>? headers})
      : headers = headers ?? const {};

  static AetherLinkFetchRequestPayload parse(Object? args) {
    if (args is! Map) {
    ...
    return AetherLinkFetchRequestPayload(url: uri, headers: headers);
  }
}

class AetherLinkFetcher {
  static const _defaultUA =
      'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';

  static Future<http.Response> _fetch(AetherLinkFetchRequestPayload payload) async {
    try {
  ...
  static Future<Map<String, dynamic>> html(AetherLinkFetchRequestPayload payload) async {
    try {
  ...
  static Future<Map<String, dynamic>> json(AetherLinkFetchRequestPayload payload) async {
    try {
      final resp = await _fetch(payload);
      final raw = resp.body;
  ...
  static Future<Map<String, dynamic>> txt(AetherLinkFetchRequestPayload payload) async {
    try {
      final resp = await _fetch(payload);
  ...
  static Future<Map<String, dynamic>> markdown(AetherLinkFetchRequestPayload payload) async {
    try {
      final resp = await _fetch(payload);
      final html = resp.body;
...
/// Minimal JSON-RPC server for MCP that serves @aetherlink/fetch tools.
class AetherLinkFetchMcpServerEngine {
  bool _closed = false;
    ...
    try {
      ...
      switch (method) {
        ...
        case mcp.McpProtocol.methodCallTool:
          ...
          AetherLinkFetchRequestPayload payload;
          try {
            payload = AetherLinkFetchRequestPayload.parse(arguments);
          } catch (e) {
            return _ok(id, result: AetherLinkFetcher._err(e.toString()));
          }

          if (name == 'fetch_html') {
            return _ok(id, result: await AetherLinkFetcher.html(payload));
          }
          if (name == 'fetch_markdown') {
            return _ok(id, result: await AetherLinkFetcher.markdown(payload));
          }
          if (name == 'fetch_txt') {
            return _ok(id, result: await AetherLinkFetcher.txt(payload));
          }
          if (name == 'fetch_json') {
            return _ok(id, result: await AetherLinkFetcher.json(payload));
          }
...
class AetherLinkInMemoryClientTransport implements mcp.ClientTransport {
  final AetherLinkFetchMcpServerEngine _server;
  final _messageController = StreamController<dynamic>.broadcast();
  final _closeCompleter = Completer<void>();
  bool _closed = false;

  AetherLinkInMemoryClientTransport(this._server);
