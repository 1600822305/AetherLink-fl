...
import 'aetherlink_fetch_server.dart';
...
/// Start the in-memory @aetherlink/fetch MCP server and connect a client to it.
...
Future<({mcp.Client client, Future<void> Function() stop})> startFetchMcpInMemory() async {
  final server = AetherLinkFetchMcpServerEngine();
  final transport = AetherLinkInMemoryClientTransport(server);

  final client = mcp.McpClient.createClient(
    mcp.McpClient.simpleConfig(name: 'AetherLink App', version: '1.0.0'),
  );
...
Future<List<(mcp.Tool tool, String id)>> listFetchTools(mcp.Client client) async {
  ...
  const serverName = '@aetherlink/fetch';
