...
class SettingsProvider extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    if (_providerConfigs.isEmpty) {
      ...
      ensureProviderConfig('AetherLinkIN', defaultName: 'AetherLinkIN');
  ...
  Future<void> setAppFontFromLocal({required String path, String? alias}) async {
    final fam = await _registerLocalFont(path: path, aliasPrefix: alias ?? 'aetherlink_local_app');
    if (fam == null) return;
    _appFontIsGoogle = false;
    _appFontFamily = fam;
  ...
  Future<void> setCodeFontFromLocal({required String path, String? alias}) async {
    final fam = await _registerLocalFont(path: path, aliasPrefix: alias ?? 'aetherlink_local_code');
    if (fam == null) return;
    _codeFontIsGoogle = false;
    _codeFontFamily = fam;
  ...
  Future<void> _reloadLocalFontsIfAny() async {
    ...
    if (_appFontLocalPath != null && _appFontLocalPath!.isNotEmpty) {
      final alias = _appFontLocalAlias ?? 'aetherlink_local_app';
    ...
    if (_codeFontLocalPath != null && _codeFontLocalPath!.isNotEmpty) {
      final alias = _codeFontLocalAlias ?? 'aetherlink_local_code';
...
class ProviderConfig {
  ...
  static String _defaultBase(String key) {
    ...
    if (k.contains('aetherlinkin')) return 'https://text.pollinations.ai/openai';
  ...
  static ProviderConfig defaultsFor(String key, {String? displayName}) {
    bool _defaultEnabled(String k) {
      ...
      if (s.contains('aetherlinkin')) return true;
    ...
    switch (kind) {
      ...
      default:
        // Special-case AetherLinkIN default models and overrides
        if (lowerKey.contains('aetherlinkin')) {
          return ProviderConfig(
            id: key,
            enabled: _defaultEnabled(key),
            name: displayName ?? key,
