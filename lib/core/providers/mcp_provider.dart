...
import '../services/mcp/aetherlink_fetch/aetherlink_fetch_server.dart';
...
class McpProvider extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    // Ensure built-in @aetherlink/fetch is present by default
  ...
  void _ensureBuiltinFetchServerPresent() {
    final exists = _servers.any((s) => s.transport == McpTransportType.inmemory || s.name == '@aetherlink/fetch' || s.id == 'aetherlink_fetch');
    if (exists) return;
      ...
      if (serversFromMap != null) {
        ...
        serversFromMap.forEach((id, cfgAny) {
          ...
          if (typeLower == 'inmemory') {
            // Built-in @aetherlink/fetch control via isActive; ignore name mismatches silently
            builtinSeen = true;
  ...
  Future<void> connect(String id) async {
    ...
    try {
      ...
      if (server.transport == McpTransportType.inmemory) {
        final engine = AetherLinkFetchMcpServerEngine();
        final transport = AetherLinkInMemoryClientTransport(engine);
