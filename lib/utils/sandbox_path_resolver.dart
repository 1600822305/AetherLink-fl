...
class SandboxPathResolver {
  ...
  static String fix(String path) {
    ...
    // - Windows: .../AppData/Local/AetherLink/<subdir>/... or .../AetherLink/<subdir>/...
    ...
    if (tail == null) {
      final int aetherlinkIdx = raw.indexOf('/aetherlink/');
      if (aetherlinkIdx != -1) {
        final candidateTail = raw.substring(aetherlinkIdx + '/aetherlink'.length); // includes leading '/'
        if (subdirs.any((s) => candidateTail.startsWith('/$s/'))) {
          tail = candidateTail;
          rootType = 'windows_aetherlink';
        }
      }
    }
