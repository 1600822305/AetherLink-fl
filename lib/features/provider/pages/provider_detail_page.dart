...
class _ProviderDetailPageState extends State<ProviderDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    bool _isUserAdded(String key) {
      const fixed = {
        'AetherLinkIN', 'OpenAI', 'Gemini', 'SiliconFlow', 'OpenRouter',
  ...
  Widget _buildConfigTab(BuildContext context, ColorScheme cs, AppLocalizations l10n) {
    return ListView(
      padding: const EdgeInsets.fromLTRB(16, 12, 16, 16),
      children: [
        if (widget.keyName.toLowerCase() == 'aetherlinkin') ...[
          Container(
        ...
        _iosSectionCard(children: [
          if (widget.keyName.toLowerCase() != 'aetherlinkin') _providerKindRow(context),
          _iosRow(
            context,
        ...
        _inputRow(
          ...
          enabled: widget.keyName.toLowerCase() != 'aetherlinkin',
          onChanged: (_) => _save(),
        ),
        const SizedBox(height: 12),
        if (!(_kind == ProviderKind.google && _vertexAI)) ...[
          if (widget.keyName.toLowerCase() != 'aetherlinkin' && !_multiKeyEnabled) ...[
            _inputRow(
              context,
        ...
        if (_kind == ProviderKind.openai && widget.keyName.toLowerCase() != 'aetherlinkin' && !_useResp) ...[
          const SizedBox(height: 12),
          _inputRow(
            context,
