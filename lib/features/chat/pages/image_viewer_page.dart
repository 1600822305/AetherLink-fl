...
class _ImageViewerPageState extends State<ImageViewerPage> with TickerProviderStateMixin {
  ...
  Future<void> _saveCurrent() async {
    ...
    try {
      ...
      final name = 'aetherlink-${DateTime.now().millisecondsSinceEpoch}';
    ...
    try {
      ...
      if (src.startsWith('data:')) {
        ...
        if (i != -1) {
          ...
          temp = await File(p.join(tmp.path, 'aetherlink_${DateTime.now().millisecondsSinceEpoch}.png')).create(recursive: true);
        ...
        if (resp.statusCode >= 200 && resp.statusCode < 300) {
          ...
          temp = await File(p.join(tmp.path, 'aetherlink_${DateTime.now().millisecondsSinceEpoch}${ext.isNotEmpty ? ext : '.jpg'}')).create(recursive: true);
  ...
  Future<void> _saveCurrentDesktop() async {
    ...
    try {
      ...
      final defaultName = 'aetherlink-${DateTime.now().millisecondsSinceEpoch}$ext';
